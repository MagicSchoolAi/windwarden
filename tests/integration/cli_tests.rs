...
fn test_exclude_patterns() {
    ...
    fs::create_dir_all(temp_dir.path().join("node_modules")).expect("Failed to create node_modules");
    fs::write(
...
fn test_max_depth_option() {
    ...
    fs::create_dir_all(temp_dir.path().join("level1/level2/level3")).expect("Failed to create nested dirs");
...
fn test_progress_flag() {
    ...
    for i in 1..=8 {
        fs::write(
            ...
            format!(r#"export const Test{} = () => <div className="p-4 bg-red-500 flex">Test{}</div>;"#, i, i),
        ).expect("Failed to write test file");
    }
...
fn test_stdin_processing() {
    let mut cmd = Command::cargo_bin("windwarden").unwrap();
    cmd.arg("--stdin")
        .write_stdin(r#"export const Test = () => <div className="p-4 bg-red-500 flex">Test</div>;"#)
        .assert()
        .success()
...
fn test_invalid_command_combinations() {
    ...
    let mut cmd = Command::cargo_bin("windwarden").unwrap();
    cmd.arg("format").arg("--mode").arg("check")
        .assert()
