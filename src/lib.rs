...
pub enum WindWardenError {
    ...
    SortError { context: String, message: String },
    ...
    BatchProcessing { file_count: usize, summary: String },
...
pub fn process_stdin_with_config(
    options: ProcessOptions,
    config: &crate::config::Config,
) -> Result<String> {
    let mut input = String::new();
...
pub fn process_files_with_windwarden_config(
    ...
    let pipeline =
        FileProcessingPipeline::new_with_windwarden_config(file_config, windwarden_config, mode)?;
    pipeline.process_files(paths, options)
}
