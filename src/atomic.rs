...
impl AtomicWriter {
    ...
    pub fn new(target_path: impl AsRef<Path>) -> Result<Self> {
        ...
        if let Some(parent) = target_path.parent() {
            fs::create_dir_all(parent)
                .map_err(|e| WindWardenError::from_io_error(e, Some(&parent.display().to_string())))?;
        }
        
        // Create the temporary file
        let temp_file = fs::File::create(&temp_path)
            .map_err(|e| WindWardenError::from_io_error(e, Some(&temp_path.display().to_string())))?;
    ...
    pub fn write(&mut self, content: &str) -> Result<()> {
        let file = self.temp_file.as_mut()
            .ok_or_else(|| WindWardenError::internal_error("AtomicWriter already finalized"))?;
        
        file.write_all(content.as_bytes())
            .map_err(|e| WindWardenError::from_io_error(e, Some(&self.temp_path.display().to_string())))?;
...
pub mod atomic {
    ...
    pub fn write_file_with_backup(path: impl AsRef<Path>, content: &str) -> Result<()> {
        ...
        if path.exists() {
            ...
            fs::copy(path, &backup_path)
                .map_err(|e| WindWardenError::from_io_error(e, Some(&backup_path.display().to_string())))?;
        }
    ...
    fn create_backup_path(path: &Path) -> Result<PathBuf> {
        ...
        for i in 1..=999 {
            let backup_name = format!("{}.bak{}", file_name, if i == 1 { String::new() } else { format!(".{}", i) });
        ...
        Err(WindWardenError::internal_error("Unable to create backup file - too many existing backups"))
    }
}
