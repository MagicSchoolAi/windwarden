...
impl Reporter {
        ...
        match self.format {
            OutputFormat::Json => {
                let json = serde_json::to_string_pretty(report)
                    .map_err(|e| WindWardenError::internal_error(format!("Failed to serialize diagnostic report: {}", e)))?;
    ...
    pub fn report_check(&mut self, report: &CheckReport) -> Result<()> {
        match self.format {
            OutputFormat::Json | OutputFormat::Check => {
                let json = serde_json::to_string_pretty(report)
                    .map_err(|e| WindWardenError::internal_error(format!("Failed to serialize check report: {}", e)))?;
                writeln!(self.writer, "{}", json)
                    .map_err(|e| WindWardenError::internal_error(format!("Failed to write output: {}", e)))?;
            }
            OutputFormat::Text => {
    ...
    pub fn report_diff(&mut self, report: &DiffReport) -> Result<()> {
        match self.format {
            OutputFormat::Json | OutputFormat::Diff => {
                let json = serde_json::to_string_pretty(report)
                    .map_err(|e| WindWardenError::internal_error(format!("Failed to serialize diff report: {}", e)))?;
                writeln!(self.writer, "{}", json)
                    .map_err(|e| WindWardenError::internal_error(format!("Failed to write output: {}", e)))?;
            }
            OutputFormat::Text => {
    ...
    pub fn report_summary(&mut self, report: &SummaryReport) -> Result<()> {
        match self.format {
            OutputFormat::Json | OutputFormat::Summary => {
                let json = serde_json::to_string_pretty(report)
                    .map_err(|e| WindWardenError::internal_error(format!("Failed to serialize summary report: {}", e)))?;
                writeln!(self.writer, "{}", json)
                    .map_err(|e| WindWardenError::internal_error(format!("Failed to write output: {}", e)))?;
            }
            OutputFormat::Text => {
    ...
    fn write_text_diagnostics(&mut self, report: &DiagnosticReport) -> Result<()> {
            ...
            if !file_result.messages.is_empty() {
                ...
                for issue in &file_result.messages {
                    ...
                    if !issue.source.is_empty() {
                        writeln!(self.writer, "    > {}", issue.source)
                            .map_err(|e| WindWardenError::internal_error(format!("Failed to write output: {}", e)))?;
                    }
                }
            }
        ...
        if report.error_count > 0 || report.warning_count > 0 {
            ...
            writeln!(self.writer, "✓ No problems found")
                .map_err(|e| WindWardenError::internal_error(format!("Failed to write output: {}", e)))?;
        }
    ...
    fn write_text_check(&mut self, report: &CheckReport) -> Result<()> {
        if report.unformatted_files.is_empty() {
            writeln!(self.writer, "All files are properly formatted!")
                .map_err(|e| WindWardenError::internal_error(format!("Failed to write output: {}", e)))?;
        } else {
            writeln!(
                self.writer,
            ...
            for file in &report.unformatted_files {
                writeln!(self.writer, "  {}", file.file_path)
                    .map_err(|e| WindWardenError::internal_error(format!("Failed to write output: {}", e)))?;
            }
        }
    ...
    fn write_text_diff(&mut self, report: &DiffReport) -> Result<()> {
        if report.changes.is_empty() {
            writeln!(self.writer, "No changes needed - all files are properly formatted!")
                .map_err(|e| WindWardenError::internal_error(format!("Failed to write output: {}", e)))?;
        } else {
            for file_changes in &report.changes {
                ...
                for modification in &file_changes.modifications {
                    ...
                    writeln!(self.writer, "    - {}", modification.original_text)
                        .map_err(|e| WindWardenError::internal_error(format!("Failed to write output: {}", e)))?;
                    writeln!(self.writer, "    + {}", modification.new_text)
                        .map_err(|e| WindWardenError::internal_error(format!("Failed to write output: {}", e)))?;
                }
            }
        }
