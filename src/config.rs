...
impl ConfigManager {
    ...
    pub fn load_config_file(path: &Path) -> Result<Config, WindWardenError> {
        ...
        let config: Config = serde_json::from_str(&content)
            .map_err(|e| WindWardenError::config_error(format!(
    ...
    fn validate_config(config: &Config) -> Result<(), WindWardenError> {
        ...
        if !["official", "custom"].contains(&config.sort_order.as_str()) {
            return Err(WindWardenError::config_error(
                format!("Invalid sort_order '{}'. Must be 'official' or 'custom'", config.sort_order)
            ));
        }
        ...
        if config.sort_order == "custom" {
            ...
            let available_set: std::collections::HashSet<&String> = available_categories.iter().collect();
            
            for category in &config.custom_order {
                if !available_set.contains(category) {
                    return Err(WindWardenError::config_error(
                        format!("Unknown category '{}' in custom_order. Available categories: {}", 
                               category, 
        ...
        if !valid_presets.contains(&config.preset_regex.as_str()) {
            return Err(WindWardenError::config_error(
                format!("Invalid preset_regex '{}'. Valid options: {}", 
                       config.preset_regex, 
                       valid_presets.join(", "))
            ));
        }
        ...
        for regex_pattern in &config.custom_regex {
            if let Err(e) = regex::Regex::new(regex_pattern) {
                return Err(WindWardenError::config_error(
                    format!("Invalid custom regex '{}': {}", regex_pattern, e)
                ));
            }
        }
        ...
        for func_name in &config.function_names {
            ...
            if func_name.contains(char::is_whitespace) {
                return Err(WindWardenError::config_error(
                    format!("Function name '{}' cannot contain whitespace", func_name)
                ));
            }
        }
        ...
        if config.threads > 1024 {
            return Err(WindWardenError::config_error(
                format!("Thread count {} is too high (max: 1024)", config.threads)
            ));
        }
        ...
        if let Some(ref mode) = config.default_mode {
            ...
            if !valid_modes.contains(&mode.as_str()) {
                return Err(WindWardenError::config_error(
                    format!("Invalid default_mode '{}'. Valid options: {}", 
                           mode, 
                           valid_modes.join(", "))
                ));
            }
    ...
    pub fn create_default_config(path: &Path) -> Result<(), WindWardenError> {
        ...
        let content = serde_json::to_string_pretty(&config)
            .map_err(|e| WindWardenError::config_error(format!("Failed to serialize default config: {}", e)))?;
    ...
    pub fn should_process_extension(&self, extension: &str) -> bool {
        self.config.file_extensions.iter().any(|ext| ext == extension)
    }
